# Alerta específico — hepatite_b_hbv × CUIABA

**Semana epidemiológica:** 2026-SE30
**Destinatários:** área técnica · Vigilância Epidemiológica (VE) municipal · CIEVS

> **Aviso:** este documento é sinal para investigação. **Não declara surto nem epidemia automaticamente.**

## 1. Evidência

- Agravo/alvo: **hepatite_b_hbv**
- Município: **CUIABA**
- Exames: 21 · positivos: 21
- Positividade: 1.0
- Variação: caiu 23% em relação à semana anterior
- Probabilidade: alto · Impacto: médio · Confiança: Derivado
- Veredito operacional: investigar

## 2. Interpretação de marcadores

- IgM (Enzimaimunoensaio): 33 positivos / 148 exames — IgM: sugere infecção recente — sinal para investigação.
- IgM (Enzimaimunoensaio): 5 positivos / 21 exames — IgM: janela de infecção recente — sinal laboratorial ativo.
- IgM (Enzimaimunoensaio): 4 positivos / 42 exames — IgM: sugere infecção recente — sinal para investigação.
- IgM (Imunoensaio de Micropartículas por Quimioluminescência): 1 positivos / 8 exames — IgM: janela de infecção recente — sinal laboratorial ativo.
- IgM (Enzimaimunoensaio): 0 positivos / 5 exames — IgM: sugere infecção recente — sinal para investigação.
- HBeAg (Imunoensaio de Micropartículas por Quimioluminescência): 0 positivos / 4 exames — HBeAg: marcador de replicação — contextualizar com DNA e clínica.
- anti_HBc_IgM (Imunoensaio de Micropartículas por Quimioluminescência): 0 positivos / 4 exames — Anti-HBc IgM: compatível com infecção aguda (ou reativação) — sinal laboratorial ativo para a VE.
- HBsAg (Imunoensaio de Micropartículas por Quimioluminescência): 0 positivos / 4 exames — HBsAg: infecção ativa (aguda ou crônica) — sinal laboratorial; cruzar com clínica, IgM e notificação (não declarar surto só com isso).

## 3. Taxas e local

- CUIABA: +21.0 (1.0) · 3,0 positivos por 100 mil habitantes

## 4. Vizinhos (a partir do município do sinal)

- VARZEA GRANDE ↔ CUIABA: +3.0/+0.0 (exames 409.0/99.0)
- CUIABA ↔ VARZEA GRANDE: +0.0/+3.0 (exames 99.0/409.0)
- CUIABA ↔ SANTO ANTONIO DO LEVERGER: +0.0/+0.0 (exames 99.0/4.0)
- CUIABA ↔ NOSSA SENHORA DO LIVRAMENTO: +0.0/+0.0 (exames 99.0/6.0)

## 5. Internações (SIH)

- CUIABA × dengue_arbovirose: n=12 [SIH/VW_INTERNACAO]

## 6. Lacunas de vigilância

- GAL×SINAN: gal_sem_sinan — 99.0 exames / 0.0 notificações (dengue)
- GAL×SINAN: gal_sem_sinan — 69.0 exames / 0.0 notificações (tuberculose)
- GAL×SINAN: gal_sem_sinan — 35.0 exames / 0.0 notificações (hepatite)
- GAL×SINAN: gal_sem_sinan — 17.0 exames / 0.0 notificações (chikungunya)
- GAL×SINAN: gal_sem_sinan — 13.0 exames / 0.0 notificações (hansen_ase_hansen_ase)
- SINAN: Notificação compulsória — vínculo mun×agravo com GAL [presente]
- GAL: Exames LACEN — demanda e positividade [presente]
- SIH: Proxy SIH via VW_INTERNACAO — internacoes correlatas (CID×mun) [presente]
- SIA: Produção ambulatorial correlata (SIA/SIA_APAC) [presente]
- SIVEP/SRAG: Base respiratória/SRAG listada só como inventário — não entra como seção padrão do Radar salvo sinal explícito. [presente]
- SIM: Óbitos — letalidade contextual [presente]

## 7. Ações sugeridas

- CIEVS: Priorizar investigação de hepatite_b_hbv em CUIABA: cruzar lab × notificação, avaliar definição de caso e comunicação com VE municipal.
- VE municipal: Investigar casos/contatos de hepatite_b_hbv em CUIABA; verificar oportunidade de notificação e busca ativa.
- Área técnica: Validar critérios clínico-laboratoriais de hepatite_b_hbv; orientar rede sobre definição de caso.
- Vizinhos: —
- LACEN: Apoiar hepatite_b_hbv: priorizar TAT/confirmação se fila impactar a VE deste agravo; comunicar resultados críticos à VE/CIEVS.

## 8. Disclaimer

Radar LACEN / CIEVS — sinal operacional. A declaração de surto, epidemia ou emergência cabe à VE após definição de caso, investigação e critérios do Guia MS.

_Gerado em 2026-08-31 10:07 Hora Padrão Brasil Central_
